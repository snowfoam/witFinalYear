...
router.post('/course/add', authUser, async (req, res) => {
  ...
  if (userType === 'student') {
    ...
    return res.status(400).json({ success: false, message: 'User type error' });
  }

  var user = await User.findOne({ courses: { $all: [courseId] } })
  if (user && user._id.toString() !== _id) {
    return res.status(401).json({ success: false, message: 'Unavaliable.' })
  }

  var user = await User.findOne({ _id })
  if (user && user.courses.includes(courseId)) {
    return res.status(401).json({ success: false, message: 'Already added.' })
  }
...
router.post('/course/cancle', authUser, async (req, res) => {
  ...
  return res.status(200).json({ success: true, message: "Course cancled." })
});
